...
emoji_data = {8:"\uD83D\uDE0F",9:"\uD83D\uDE4F",10:"\uD83D\uDE07",11:"\uD83D\uDE03",12:"\uD83D\uDE08",13:"\uD83D\uDE0A",14:"\uD83D\uDE0C",0:u"\uD83D\uDE01",1: u"\uD83D\uDE02", 2:"\uD83D\uDE04", 3:"\uD83D\uDE05",
                 4:"\uD83D\uDE06", 5:"\uD83D\uDE09",6:"\uD83D\uDE0B", 7:"\uD83D\uDE0D"}

def send_message(mssg):
    for a in range(len(emoji_data)):
        new_str = "eren{}mhs".format(a)
        if emoji_data[a] in mssg:
            return str(mssg).replace(emoji_data[a], new_str)
    return mssg
...
def recieve_message(mssg):
    ...
    for a in range(len(emoji_data)):
        new_str = "eren{}mhs".format(a)
        if new_str in mssg:
            return str(mssg).replace(new_str,emoji_data[a])
    return mssg
...
class Chat_GUI:
    def __init__(self,connection, ip, selected_prot):
        ...
        self.selectemoji = tk.Button(self.base, font="Helvetica", text="\uD83D\uDE01", width="40", height=5,
                                    bd=0, activebackground="#BDE096", justify="center",
                                    command=self.generate_emojies)
        ...
        self.selectemoji.place(x=310, y=390, height=50, width=50)
    ...
    def generate_emojies(self):

        self.base1 = tk.Tk()
        self.base1.title("Select an Emoji")
        self.base1.geometry("550x150")
        self.button_list = [i for i in range(len(emoji_data))]
        for k, j in emoji_data.items():
            self.button_list[k] = tk.Button(self.base1, font="Helvetica", text=j, bd=0, activebackground="#BDE096",
                                            justify="left",command=partial(self.generate_emoji_index,k)).pack(side="left")

    def generate_emoji_index(self,ind):
        self.textBox.insert('end',emoji_data[ind])
class Main_GUI(tk.Frame):
    def __init__(self, parent):
        self.parent = parent
        tk.Label(self.parent, text="Eren and Mahmut Chat Tool v1.0", bg='lavender', fg="black",
        ...
        self.destinationIP.insert(tk.END,'192.168.43.134')
...
def main():
    root = tk.Tk()
    root.title("EM Chat Tool v1.0--Client")
